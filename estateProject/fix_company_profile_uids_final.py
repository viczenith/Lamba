...
print("=" * 100)
print("FIXING ALL COMPANY PROFILE UIDs (HANDLING DUPLICATES)")
print("=" * 100)
...
for company in companies:
    ...
    # Delete existing profiles to regenerate
    old_count = CompanyClientProfile.objects.filter(company=company).count()
    CompanyClientProfile.objects.filter(company=company).delete()
    print(f"  → Cleared {old_count} old profiles")
    ...
    for idx, client_id in enumerate(all_client_ids, start=1):
        ...
        print(f"  ✓ {new_uid:15} - {client.full_name}")
    
    # ========== MARKETERS ==========
    print(f"\n▶ FIXING MARKETER UIDs:")
    
    # Delete existing profiles to regenerate
    old_count_m = CompanyMarketerProfile.objects.filter(company=company).count()
    CompanyMarketerProfile.objects.filter(company=company).delete()
    print(f"  → Cleared {old_count_m} old profiles")
    ...
    for idx, marketer_id in enumerate(all_marketer_ids, start=1):
        ...
        print(f"  ✓ {new_uid:15} - {marketer.full_name}")
